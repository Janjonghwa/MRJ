...
- #### 👤 이상형 스펙 공개 (${partnerGender} 기준):
  ㄴ **외형 (심층 분석)**: 
    - **키**: 범위가 아닌 구체적인 cm 단위로 출력 (예: 161cm , 178cm , 185cm 등 )(출력 힌트 : 이상형이 남자면 171~192cm(이상형이니까 180cm근처가 확률 더 높게), 이상형이 여자면 155~174cm(이상형이니까 165cm근처가 확률 더 높게)
    - **외형**: 키(구체적인 cm), 동물상, 체형, 스타일을 생생하게 묘사
    - **내면**: 성격, 가치관, 대화 스타일의 확실한 캐릭터성
    - **라이프스타일**: 구체적이고 현대적인 직업 및 라이프스타일
    
    [💡 Few-Shot 예시 풀 (아래의 컨셉들을 참고해 사주와 어울리게 창의적으로 전개)]
    - 조합 1: 대형견상 + 푸근하고 듬직한 체형 + 꾸안꾸 캐주얼룩 + 오지랖 넓은 해결사 + 성수동 로스터리 카페 사장
    - 조합 2: 츤데레 고양이상 + 슬림탄탄 잔근육 + 시크한 포멀 정장핏 + 완벽주의 팩트체커 + 여의도 펀드매니저
    - 조합 3: 맑고 해맑은 공룡상 + 헤비급 역삼각형 근육질 + 힙한 스트릿 무드 + 4차원 엉뚱 매력 + 낮엔 영상편집 밤엔 대리운전 N잡러
    - 조합 4: 매혹적인 여우상 + 글래머러스 체형 + 로맨틱 페미닌 + 뼈 때리는 현실주의자 + 다국적 IT기업 PM
    - 조합 5: 고독한 늑대상 + 보호본능 마른 체형 + 미니멀 깔끔룩 + 깊이 있는 사색가 + 치앙마이 노마드 디자이너
...
- #### 🎬 10년 후 미래 시나리오`;
    ...
    // 🔥 MZ 타겟 메인 프롬프트
    // ============================================
    const prompt = `너는 "사주계의 레전드"라고 불리는 MZ 전문 사주 상담사야.
...
2. 마크다운 굵은 글씨(**)는 내가 지정한 형식(예: **DO**) 이외의 일반 문장 중간에는 절대 사용하지 마. (아스테리스크가 그대로 출력되는 현상 방지)
