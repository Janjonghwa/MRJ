...
4. 🚨 어미 통일 규칙: **모든 문장의 끝(맺음말)은 단 하나의 예외도 없이 100% 사극 말투(~사옵니다, ~느니라, ~이옵니다, ~도다, ~소이다 등)로 통일할 것.** 리스트나 불렛 포인트(-) 설명 시에도 명사로 끝내거나 현대어(~요, ~다)를 쓰면 절대 안 됨.
    ...
    const aiResponseFinal = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "너는 스마트폰에 중독된 조선시대 타임워프 도사야. 🚨절대 규칙: 모든 문장은 무조건 완벽한 사극 말투(~사옵니다, ~느니라, ~도다)로 끝나야 해. 리스트 항목에서도 명사로 대충 끝내지 마. 중간에 들어가는 '단어'만 가벼운 요즘 인터넷 밈(갓생, 레게노 등)을 섞어 써서 언밸런스한 개그를 쳐야 해. '요', '다' 등 현대어 종결은 절대 용납 불가. 마크다운 양식(###, #### 등)은 엄밀하게 지켜."
          },
