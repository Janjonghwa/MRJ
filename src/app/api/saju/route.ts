...
- #### 👤 이상형 스펙 공개 (${partnerGender} 기준):
  ㄴ **외형 (심층 분석)**: 
    ...
    [💡 묘사 포맷 및 무한 생성 가이드 (헤비유저 반복 테스트 대비)]
    AI의 고질적인 '뻔한 답변 반복(예: 잔근육, 스타트업 PM, 플리마켓 등)'을 금지함. 유저가 100번을 테스트해도 매번 완전히 새로운 결과가 나오도록, 아래의 **[생성 공식]**을 바탕으로 세상에 존재하는 수만 가지 단어 중 가장 디테일하고 트렌디한 것을 매번 새롭게 발굴하여 조합할 것. (아래 예시 단어를 그대로 쓰지 말고 방향성만 참고할 것!)

    **1. [외모/동물상] 생성 공식**
    - 흔한 강아지, 고양이, 여우도 가능하지만 사주의 기운을 담아 구체적이고 유니크한 동물을 끌어올 것
    - (생성 방향성 예시: 나른한 친칠라상, 맑은 눈의 알파카상, 예민한 아기 호랑이상, 우아한 흑조상, 서늘한 흑표범상, 털복숭이 포메라니안상, 듬직한 곰상 등)
    ...
    **3. [구체적 직업군] 생성 공식**
    - '마케터, PM, 개발자' 같은 대분류 금지. 링크드인(LinkedIn)에서 볼 법한 [구체적인 타겟/산업군 + 세부 직무]를 결합할 것.
    - (생성 방향성 예시: 피부과 의사,회계법인 다니는 회계사,B2B SaaS 프로덕트 디자이너, 하이엔드 니치향수 브랜드 조향사, 로컬 F&B 팝업 기획자, 지속가능성 소재 연구원, 방산기업 연구원, K-pop 아이돌 무대 연출가 등)

    **4. [만나는 장소] 생성 공식**
    - '플리마켓, 힙한 카페, 데이트 앱' 절대 금지. [특정 시간/상황 + 취향이 담긴 구체적 장소]로 씬(Scene)을 그릴 것.
    - (생성 방향성 예시: 토요일 아침 오픈런하는 한정판 스니커즈 매장, 비 오는 금요일 밤의 언더그라운드 재즈바, 심야 러닝 크루 하프 마라톤 준비 모임, 독립서점의 철학 세미나 뒤풀이 등)

- #### 📅 인연 터지는 시기 (연도/월 단위로)
- #### 📍 어디서 만남? (위의 **[만나는 장소] 생성 공식**을 적용하여 구체적으로 작성)
- #### 🎬 첫 만남 뇌피셜 씬(Scene)
  앞서 생성한 [이상형 스펙(동물상+스타일+직업)]과 [만나는 장소]를 하나로 엮어서, 마치 로맨스 웹드라마나 영화의 한 장면처럼 생생하고 설레는 2~3문장의 스토리텔링을 작성할 것. 유저가 과몰입할 수 있게 구체적인 행동이나 대화를 살짝 곁들일 것. 
  (방향성 예시: "심야 러닝 크루 하프 마라톤 준비 모임에서 페이스 조절을 못 해 뒤처진 당신. 그때, 고프코어룩을 힙하게 입은 '맑은 눈의 알파카상' 지속가능성 소재 연구원이 다가와 옆에서 묵묵히 속도를 맞춰주며 플러팅이 시작됩니다.")
...
9. "여우상+잔근육"처럼 특정 패턴만 앵무새처럼 반복하지 마. 매력적인 이상형의 범주를 지키되, 너무 판타지스럽거나 이상한 조합은 피하고 '현실에서 인기가 많을 법한 트렌디한' 동물상, 체형, 직업, 스타일을 매번 무작위 배합해.
