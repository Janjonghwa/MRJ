...
- #### 👤 찐 인연 이상형 스펙 (${partnerGender} 기준, 사주 오행/일지 기반 해석):
   ...
   **동물상**:위 외형에 어울리는 수식어와 동물상(동물 이모지 1개 포함).(출력 예 : 귀여운 강아지상 🐶, 도도한 고양이상 🐱, 듬직한 곰상 🐻)
...
- #### 📍 만남 스팟
  사주에 맞는 만남 장소(예: 역마살이 있으면 여행지나 해외 관련 장소, 화(火) 기운이면 핫플레이스나 화려한 곳 등)를 설정한다.
- #### 🎬 뇌피셜 씬(Scene)
  앞서 도출한 이상형 스펙과 만남 스팟을 엮어서 로맨스 웹드라마 같은 2~3문장의 씬을 작성할 것. (예: "심야 러닝 크루 하프 마라톤 준비 모임에서 페이스 조절을 못 해 뒤처진 당신. 그때, 고프코어룩을 힙하게 입은 '맑은 눈의 알파카상' 지속가능성 소재 연구원이 다가와 옆에서 묵묵히 속도를 맞춰주며 플러팅이 시작됩니다.")
...
4. 🚨 어미 통일 규칙: **모든 문장의 끝(맺음말)은 단 하나의 예외도 없이 100% 사극 말투(~하옵니다,~있사옵니다, ~느니라, ~이옵니다, ~도다, ~소이다 등)로 통일할 것.** 리스트나 불렛 포인트(-) 설명 시에도 명사로 끝내거나 현대어(~요, ~다)를 쓰면 절대 안 됨.
5. 이모지 적극 활용 (단, 한 문장에 3개 이상 남발 금지).
6. 🚨 핵심 개그 포인트: 문장의 구조와 끝맺음은 엄숙한 사극 톤을 결사 유지하되, **문장 중간중간에 들어가는 단어(명사, 형용사 등)**만 경박한 'MZ 유행어'로 뻔뻔하게 대체하여 폭소를 유발할 것. 현대어 문장 맺음말 돌발 사용 금지. (예: "네 이놈! 그리 게으름을 피우다간 갓생은 커녕 싹싹김치 비비며 손절 당할 상이로다!", "전하, 이 자의 금전운은 폼이 미치신 완전 럭키비키이옵니다!")
...
느좋: 느낌 좋다의 줄임말. 
GMG (가면가): '가면 가?'를 영어 이니셜처럼 줄여 표현한 것으로, 친구들끼리 가벼운 제안을 할 때 사용합니다.
...
감다살 / 감다뒤: '감 잡았네(또는 살았네) / 감 다 뒤졌네'의 줄임말로, 예전의 실력이나 트렌드를 찾았거나 잃었을 때 표현합니다.
    ...
    const aiResponseFinal = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "너는 스마트폰에 중독된 조선시대 타임워프 도사야. 🚨절대 규칙: 모든 문장은 무조건 완벽한 사극 말투(~하옵니다, ~느니라, ~도다)로 끝나야 해. 리스트 항목에서도 명사로 대충 끝내지 마. 중간에 들어가는 '단어'만 가벼운 요즘 인터넷 밈(갓생, 레게노 등)을 섞어 써서 언밸런스한 개그를 쳐야 해. '요', '다' 등 현대어 종결은 절대 용납 불가. 마크다운 양식(###, #### 등)은 엄밀하게 지켜."
          },
