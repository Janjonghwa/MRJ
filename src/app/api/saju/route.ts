...
- #### 👤 이상형 스펙 공개 (${partnerGender} 기준):
  ㄴ **외형 (심층 분석)**: 
    ...
    - **비주얼**: 동물상(해당 동물 이모지 같이 출력), 체형, 스타일을 생생하게 묘사.
      (🚨경고: "여우상 🦊", "잔근육" 등 특정 표현만 무한 반복하지 말 것. 토끼상 🐰, 사슴상 🦌, 늑대상 🐺, 강아지상 🐶, 곰상 🐻, 고양이상 🐱 등 여러 매력적인 동물상 중 다양하게 쓸 것. 
      단, 지나치게 만화 같거나 난해한 묘사(예: 돌고래상, 네온 서프보드룩 등)는 절대 금지! 인스타그램이나 브이로그에서 볼 법한 '현실적으로 훈훈하고 매력적인' 체형(예: 슬림탄탄, 어깨가 넓은 듬직한 핏, 비율이 좋은 모델핏 등)과 스타일(올드머니룩, 미니멀룩, 꾸안꾸 캐주얼, 포멀룩 등) 내에서 무작위로 조합할 것)
    - **내면**: 성격, 가치관, 대화 스타일의 확실한 캐릭터성
    - **라이프스타일**: 구체적이고 현실적인 직업 및 라이프스타일
    
    [💡 묘사 포맷 가이드 (아래의 구조를 참고하되, '현실적이면서 트렌디한 매력'을 유지하며 매번 다른 조합을 만들 것)]
    - 포맷: [동물상 이모지] + [매력적인 체형] + [현실적인 트렌디 스타일링] + [성격/기질] + [구체적 직업]
    (예시 구조 1: 무쌍의 담백한 늑대상 🐺 + 어깨가 넓은 듬직한 핏 + 깔끔한 미니멀룩 + 겉바속촉 다정한 성격 + 판교 IT 기획자)
    (예시 구조 2: 맑고 똘망똘망한 사슴상 🦌 + 여리여리하지만 탄탄한 핏 + 세련된 올드머니룩 + 꼼꼼하고 지적인 성향 + 외국계 기업 마케터)
