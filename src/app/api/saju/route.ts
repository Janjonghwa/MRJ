...
야르: 기분이 아주 좋거나, 성취감을 느꼈을 때 외치는 '앗싸!', '오예!', 'Yes!'와 비슷한 감탄사
꾸안꾸: '꾸민 듯 안 꾸민 듯'의 줄임말. 과하지 않고 자연스럽게 스타일링된 상태를 뜻합니다. (예: "오늘 너 꾸안꾸 완전 성공했다.")
스불재: '스스로 불러온 재앙'의 줄임말. 자기가 자초한 힘든 상황을 자조적으로 표현할 때 씁니다. (예: "야근? 내가 먼저 하겠다고 했으니까 완전 스불재지.")
존버: '존X 버티다'의 줄임말. 어떤 힘든 상황에서도 끝까지 버텨낸다는 의지를 표현합니다. (예: "취업 될 때까지 그냥 존버해야지.")
탕진잼: '탕진(재산을 다 써버리는 것)' + '잼(재미)'의 합성어. 거창한 소비가 아닌 소소한 쇼핑에 돈을 아낌없이 쓰며 즐거움을 느끼는 것을 뜻합니다. (예: "다이소에서 완전 탕진잼 했어.")
갓벽하다: 'God(신)' + '완벽하다'의 합성어. 완벽함을 넘어 신의 경지에 이르렀다는 최고의 칭찬입니다. (예: "이 조합 진짜 갓벽하다.")
현타: '현실 자각 타임'의 줄임말. 신나게 놀다가 혹은 무언가를 하다가 갑자기 현실이 직격으로 느껴지는 허탈한 순간을 뜻합니다. (예: "여행 마지막 날 현타 미쳤다.")
뇌절: 같은 말이나 행동을 지나치게 반복해서 상대방을 지치게 만드는 것을 뜻합니다. (예: "그 드립 세 번째인데 이제 뇌절이야.")
부캐: '부(副)캐릭터'의 줄임말. 게임 용어에서 출발해, 현실에서 본업 외에 새로운 정체성이나 활동을 가지는 것을 뜻합니다. (예: "요즘 부캐로 유튜브 시작했어.")
캐박이다: 어떤 물건이나 콘텐츠가 터무니없이 가성비가 좋거나 완성도가 높을 때 쓰는 표현입니다. (예: "이 가격에 이 퀄리티? 완전 캐박이다.")
과몰입: 드라마, 게임, 아이돌 등 특정 대상에 지나칠 정도로 깊이 빠져드는 것을 뜻합니다. 자조적이면서도 애정 어린 뉘앙스로 씁니다. (예: "이 드라마에 과몰입해서 밤새 다 봤어.")
별다줄: '별걸 다 줄인다'의 줄임말. 이 단어 자체가 줄임말 문화를 비꼬는 메타적인 신조어입니다. (예: "스불재? 별다줄이네 진짜.")
영끌: '영혼까지 끌어모으다'의 줄임말. 가진 돈을 전부 긁어모아 무언가에 투자하거나 구매하는 것을 뜻합니다. (예: "영끌해서 콘서트 티켓 샀어.")
점메추 / 저메추: '점심 메뉴 추천' / '저녁 메뉴 추천'의 줄임말. (예: "점메추 좀 해줘")
억텐: '억지 텐션'의 줄임말. 진심으로 즐겁지 않은데 분위기에 맞추려고 억지로 신난 척할 때 씁니다. (↔ 찐텐)
취저: '취향 저격'의 줄임말. 무언가가 내 취향에 딱 맞을 때 씁니다. (예: "이 노래 완전 취저야")
오히려 좋아: 안 좋은 상황이 생겼지만 긍정적으로 받아들이겠다는 뜻. 역설적 긍정 표현입니다.
어쩔티비: 상대방의 말에 반박하거나 대꾸할 때 쓰는 장난스러운 표현. "어쩌라고"의 변형으로, 뒤에 저쩔티비, 안물티비 등으로 이어지기도 합니다.
현타: '현실 자각 타임'의 줄임말. 갑자기 현실을 깨닫고 허탈하거나 우울해지는 순간을 뜻합니다.
인싸 / 아싸: 'Insider(인사이더)' / 'Outsider(아웃사이더)'의 줄임말. 사교적인 사람 / 혼자를 즐기는 사람을 뜻합니다.
핫플: 'Hot Place'의 줄임말. 요즘 뜨는 인기 장소를 뜻합니다.
존버: '존나 버티다'의 줄임말. 힘든 상황에서도 끝까지 버틴다는 뜻으로, 주로 주식이나 코인 투자에서 많이 씁니다.
띵작: '명작'을 ㅁ→ㄷ 오타로 친 것. 최고의 작품이라는 칭찬입니다. (예: "이 영화 띵작이야")
N잡러: 본업 외에 여러 가지 부업(N개의 Job)을 하는 사람을 뜻합니다.
그잡채: '그것이 실제로 일어났습니다 + 잡채'의 합성어. 믿기 힘든 일이 실제로 일어났을 때 씁니다.
소확행: '소소하지만 확실한 행복'의 줄임말. 일상 속 작은 기쁨을 소중히 여기는 가치관입니다.
가보자고: 무언가를 열정적으로 시작하자고 할 때 외치는 말. 의지와 각오를 담은 표현입니다.
